...
@interface CZYVideoBrowseActionSlider : UISlider
@end
@implementation CZYVideoBrowseActionSlider
- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        [self setThumbImage:[CZYIBFileManager getImageWithName:@"czyib_circlePoint"] forState:UIControlStateNormal];
...
@interface CZYVideoBrowseActionBar () {
    BOOL _isDragging;
}
...
@property (nonatomic, strong) CZYVideoBrowseActionSlider *slider;
@end

@implementation CZYVideoBrowseActionBar
...
- (CGRect)getFrameWithContainerSize:(CGSize)containerSize {
    CGFloat height = kActionBarDefaultsHeight + CZYIB_HEIGHT_EXTRABOTTOM;
...
- (void)clickPlayButton:(UIButton *)button {
    ...
    if (button.selected) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(czy_videoBrowseActionBar:clickPauseButton:)]) {
            [self.delegate czy_videoBrowseActionBar:self clickPauseButton:button];
        }
    } else {
        if (self.delegate && [self.delegate respondsToSelector:@selector(czy_videoBrowseActionBar:clickPlayButton:)]) {
            [self.delegate czy_videoBrowseActionBar:self clickPlayButton:button];
        }
    }
...
- (void)respondsToSliderTouchFinished:(UISlider *)slider {
    if (self.delegate && [self.delegate respondsToSelector:@selector(czy_videoBrowseActionBar:changeValue:)]) {
        [self.delegate czy_videoBrowseActionBar:self changeValue:slider.value];
    }
}
...
- (UIButton *)playButton {
    if (!_playButton) {
        ...
        [_playButton setImage:[CZYIBFileManager getImageWithName:@"czyib_play"] forState:UIControlStateNormal];
        [_playButton setImage:[CZYIBFileManager getImageWithName:@"czyib_pause"] forState:UIControlStateSelected];
...
- (CZYVideoBrowseActionSlider *)slider {
    if (!_slider) {
        _slider = [CZYVideoBrowseActionSlider new];
