...
@implementation UIView (CZYImageBrowserProgressView)

- (void)czy_showProgressViewWithValue:(CGFloat)progress {
    [self czy_showProgressView];
    [self.czy_progressView showProgress:progress];
}

- (void)czy_showProgressViewLoading {
    [self czy_showProgressView];
    [self.czy_progressView showLoading];
}

- (void)czy_showProgressViewWithText:(NSString *)text click:(nullable void (^)(void))click {
    [self czy_showProgressView];
    [self.czy_progressView showText:text click:click];
}

- (void)czy_showProgressView {
    CZYImageBrowserProgressView *progressView = self.czy_progressView;
    if (!progressView) {
        progressView = [CZYImageBrowserProgressView new];
        self.czy_progressView = progressView;
    }
...
- (void)setCzy_progressView:(CZYImageBrowserProgressView * _Nonnull)czy_progressView {
    objc_setAssociatedObject(self, "CZYImageBrowserProgressView", czy_progressView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CZYImageBrowserProgressView *)czy_progressView {
    return objc_getAssociatedObject(self, "CZYImageBrowserProgressView");
}

@end


@interface CZYImageBrowserProgressDrawView : UIView
@property (nonatomic, assign) CGFloat progress;
@end
@implementation CZYImageBrowserProgressDrawView
...
typedef NS_ENUM(NSUInteger, CZYImageBrowserProgressType) {
    CZYImageBrowserProgressTypeProgress,
    CZYImageBrowserProgressTypeLoad,
    CZYImageBrowserProgressTypeText
};

@interface CZYImageBrowserProgressView () {
    CZYImageBrowserProgressType  _type;
}
@property (nonatomic, strong) UILabel     *textLabel;
@property (nonatomic, strong) UIImageView *imageView;
@property (nonatomic, strong) CZYImageBrowserProgressDrawView *drawView;
@property (nonatomic, copy) void(^clickTextLabelBlock)(void);
@end
@implementation CZYImageBrowserProgressView
...
- (void)showProgress:(CGFloat)progress {
    self.userInteractionEnabled = NO;
    self->_type = CZYImageBrowserProgressTypeProgress;
...
- (void)showLoading {
    self.userInteractionEnabled = NO;
    self->_type = CZYImageBrowserProgressTypeLoad;
...
- (void)showText:(NSString *)text click:(void(^)(void))click {
    ...
    self->_type = CZYImageBrowserProgressTypeText;
    self.drawView.hidden = YES;
...
- (CZYImageBrowserProgressDrawView *)drawView {
    if (!_drawView) {
        _drawView = [CZYImageBrowserProgressDrawView new];
...
- (UIImageView *)imageView {
    if (!_imageView) {
        UIImage *img = [CZYIBFileManager getImageWithName:@"czyib_pround"];
