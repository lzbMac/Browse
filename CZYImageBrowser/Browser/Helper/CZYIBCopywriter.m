...
@implementation CZYIBCopywriter

+ (instancetype)shareCopywriter {
    static CZYIBCopywriter *copywriter = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        copywriter = [CZYIBCopywriter new];
    });
    return copywriter;
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        CZYIBCopywriterType type = CZYIBCopywriterTypeSimplifiedChinese;
        ...
        if (appleLanguages && appleLanguages.count > 0) {
            ...
            if (![languages isEqualToString:@"zh-Hans-CN"]) {
                type = CZYIBCopywriterTypeEnglish;
            }
        }
        self.type = type;
...
- (void)initCopy {
    BOOL en = self.type == CZYIBCopywriterTypeEnglish;
...
- (void)setType:(CZYIBCopywriterType)type {
    _type = type;
    [self initCopy];
}
