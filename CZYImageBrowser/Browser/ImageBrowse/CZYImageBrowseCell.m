...
@interface CZYImageBrowseCell()<CZYImageBrowserCellProtocol, UIScrollViewDelegate, UIGestureRecognizerDelegate> {
...
@property (nonatomic, strong) CZYImageBrowserProgressView *progressView;
@property (nonatomic, strong) CZYImageBrowseCellData *cellData;

@end

@implementation CZYImageBrowseCell
...
- (void)czy_initializeBrowserCellWithData:(id<CZYImageBrowserCellDataProtocol>)data layoutDirection:(CZYImageBrowserLayoutDirection)layoutDirection containerSize:(CGSize)containerSize {
    ...
    if (![data isKindOfClass:CZYImageBrowseCellData.class]) return;
    self.cellData = data;
...
- (void)cellDataStateChanged {
    CZYImageBrowseCellData *data = self.cellData;
    CZYImageBrowseCellDataState dataState = data.dataState;
    switch (dataState) {
        case CZYImageBrowseCellDataStateInvalid: {
            [self.contentView czy_showProgressViewWithText:[CZYIBCopywriter shareCopywriter].imageIsInvalid click:nil];
        }
            break;
        case CZYImageBrowseCellDataStateImageReady: {
            ...
            break;
        case CZYImageBrowseCellDataStateIsDecoding: {
            if (!self.mainImageView.image) {
                [self.contentView czy_showProgressViewLoading];
            }
        }
            break;
        case CZYImageBrowseCellDataStateDecodeComplete: {
            [self.contentView czy_hideProgressView];
        }
            break;
        case CZYImageBrowseCellDataStateCompressImageReady: {
            self.mainImageView.image = data.compressImage;
            [self updateMainContentViewLayoutWithContainerSize:self->_containerSize fillType:[data getFillTypeWithLayoutDirection:self->_layoutDirection]];
        }
            break;
        case CZYImageBrowseCellDataStateThumbImageReady: {
            // 如果图片已经展示，丢弃预览图
            if (!self.mainImageView.image) {
            ...
            break;
        case CZYImageBrowseCellDataStateIsCompressingImage: {
            [self.contentView czy_showProgressViewLoading];
        }
            break;
        case CZYImageBrowseCellDataStateCompressImageComplete: {
            [self.contentView czy_hideProgressView];
        }
            break;
        case CZYImageBrowseCellDataStateIsLoadingPHAsset: {
            [self.contentView czy_showProgressViewLoading];
        }
            break;
        case CZYImageBrowseCellDataStateLoadPHAssetSuccess: {
            [self.contentView czy_hideProgressView];
        }
            break;
        case CZYImageBrowseCellDataStateLoadPHAssetFailed: {
            [self.contentView czy_showProgressViewWithText:[CZYIBCopywriter shareCopywriter].imageIsInvalid click:nil];
        }
            break;
        case CZYImageBrowseCellDataStateIsDownloading: {
            [self.contentView czy_showProgressViewWithValue:data.downloadProgress];
        }
            break;
        case CZYImageBrowseCellDataStateDownloadProcess: {
            [self.contentView czy_showProgressViewWithValue:data.downloadProgress];
        }
            break;
        case CZYImageBrowseCellDataStateDownloadSuccess: {
            [self.contentView czy_hideProgressView];
        }
            break;
        case CZYImageBrowseCellDataStateDownloadFailed: {
            [self.contentView czy_showProgressViewWithText:[CZYIBCopywriter shareCopywriter].downloadImageFailed click:nil];
        }
            break;
        default:
            break;
...
- (void)updateMainContentViewLayoutWithContainerSize:(CGSize)containerSize fillType:(CZYImageBrowseFillType)fillType {
    CGSize imageSize;
...
- (void)_cutImage {
    ...
    CZYImageBrowseCellData *tmp = self.cellData;
...
- (CZYImageBrowserProgressView *)progressView {
    if (!_progressView) {
        _progressView = [CZYImageBrowserProgressView new];
    }
